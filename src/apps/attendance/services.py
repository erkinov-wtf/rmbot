...
def _attendance_rules() -> tuple[int, int, int, int, int, ZoneInfo]:
    rules = get_active_rules_config().get("attendance", {})
...
def check_in(user_id: int) -> tuple[AttendanceRecord, int]:
    ...
    append_xp_entry(
        ...
        payload={
            ...
            "timezone": str(_attendance_rules()[-1]),
        },
    )
