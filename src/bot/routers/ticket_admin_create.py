...
async def ticket_create_entrypoint(
    message: Message,
    state: FSMContext,
    _,
    user: User = None,
    ...
    if not user or not user.is_active:
        await ticket_admin_support._notify_not_registered_message(message=message, _=_)
        return
...
async def ticket_create_callback_handler(
    query: CallbackQuery,
    state: FSMContext,
    _,
    user: User = None,
):
    parsed = ticket_admin_support._parse_create_callback(callback_data=query.data or "")
    if parsed is None:
    ...
    if not permissions.can_create:
        await state.clear()
        await query.answer(_("Your roles do not allow ticket intake."), show_alert=True)
        return
    ...
    if action == "list":
        ...
        await ticket_admin_support._show_create_items_page(
            query=query, state=state, page=page, _=_
        )
        await query.answer()
        return
    ...
    if action == "go":
        if not selected_ids:
            await query.answer(_("Select at least one part first."), show_alert=True)
            return
        ...
        draft_color, draft_minutes = ticket_admin_support._draft_for_part(
            part_id=part_order[0], part_specs=part_specs
        )
        ...
        current_part_name = next(
            (str(part["name"]) for part in parts if int(part["id"]) == part_order[0]),
            f"Part #{part_order[0]}",
        )
        await ticket_admin_support._safe_edit_message(
            query=query,
    ...
    if action in {"clr", "min", "adj", "save", "create"}:
        ...
        if action == "clr":
            ...
            except ticket_admin_support.drf_serializers.ValidationError as exc:
