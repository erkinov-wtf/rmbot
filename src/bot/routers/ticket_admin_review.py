...
async def ticket_review_entrypoint(
    message: Message,
    state: FSMContext,
    _,
    user: User = None,
...
async def ticket_review_queue_callback_handler(
    query: CallbackQuery,
    state: FSMContext,
    _,
    user: User = None,
...
async def ticket_review_action_callback_handler(
    query: CallbackQuery,
    state: FSMContext,
    _,
    user: User = None,
    ...
    if action == ticket_admin_support.REVIEW_ACTION_ASSIGN_OPEN:
        ...
        technician_options, assign_page, assign_page_count, _assign_total_count = (
            await run_sync(
    ...
    if action == ticket_admin_support.REVIEW_ACTION_ASSIGN_PAGE:
        ...
        technician_options, safe_page, page_count, _assign_total_count = await run_sync(
            ticket_admin_support._list_technician_options_page,
            page=assign_page,
            per_page=ticket_admin_support.TECHNICIAN_OPTIONS_PER_PAGE,
        )
    ...
    if action == ticket_admin_support.REVIEW_ACTION_ASSIGN_EXEC:
        ...
        if arg is None:
            await query.answer(_("Technician selection is invalid."), show_alert=True)
            return
        try:
